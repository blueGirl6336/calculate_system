...
$.ajax({
		...
		success:function(data)
		{
			...
			for(var m = 0;m<personalityNum.length;m++)
			{
				dailyTitle.push(personalityTitle[m]);
			}
			for(var n = 0;n<personalityNum.length;n++)
			{
				dailyCol.push(personalityCol[n]);
			}
...
$.ajax({
		...
		success:function(data)
		{
			...
			for(var i=0;i<dataList.length;i++)
			{
				...
				// alert(personality[0]);
				for(var b = 0;b < personalityNum.length;b++)
				{
					//alert("进入个性发展分数据填充循环");

					var result = personality.indexOf(personalityNum[b]);
					//alert(k+":"+result);
					if(result < 0)
					{
						temp.put(personalityNum[b]," ");
						//alert("判断小于0");
					}else
					if(result >= 0)
					{
						temp.put(personalityNum[b],personality_score[result]);
						//alert("判断大于等于0");
					}
				}
