...
public class WinPrizeDaoImpl extends BaseDaoImpl implements WinPrizeDao {
	...
	public WinPrize searchWinPrizeById(int wpId) {
		String hql = "from WinPrize as w where w.wpId =" + wpId + " and isCurrent=1";
	...
	public List<WinPrize> searchWinPrizeByUserName(String userName) {
		String hql = "from WinPrize as w where w.student.users.userName =" + userName + " and isCurrent=1";
		List<WinPrize> winPrizeList = (List<WinPrize>) this.getHibernateTemplate().find(hql);
		return winPrizeList;
	}

	@Override
	public List<WinPrize> searchWinPrizeByApartmentId(int apartmentId) {
		String hql = "from WinPrize as w where w.isCurrent=1 and" + " w.prize.prizeId in (select prizeId from Prize as p where p.apartment.apartmentId = " + apartmentId + ")";
		List<WinPrize> winPrizeList = (List<WinPrize>) this.getHibernateTemplate().find(hql);
		return winPrizeList;
	}
	...
	public WinPrize searchByPrizeApplicationId(int paId) {
		String hql = "from WinPrize as w where w.prizeApplication.paId =" + paId + " and isCurrent=1";
		List<WinPrize> winPrizeList = (List<WinPrize>) this.getHibernateTemplate().find(hql);
		return winPrizeList.get(0);
	}
	...
	public List<WinPrize> searchAllWinPrize() {
		String hql = "from WinPrize where isCurrent=1";
		List<WinPrize> winPrizeList = (List<WinPrize>) this.getHibernateTemplate().find(hql);
		return winPrizeList;
	}
