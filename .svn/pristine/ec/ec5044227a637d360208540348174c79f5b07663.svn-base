...
public class StudentServiceImpl implements StudentService {
	...
	{
		int studentId;
		MajorRankTemp majorRankTemp = new MajorRankTemp();
		Users user = new Users();
		List<MajorRankTemp> allRankList = new ArrayList<MajorRankTemp>();
		...
		if(null != classList && 0 != classList.size())
		{
			for(Classes classItem : classList)
			{
				...
				if(null != rankList)
				{
					...
					{
						user = userDao.getUserByUserId(item.getUsers().getUserId());
						majorRankTemp = new MajorRankTemp();
						majorRankTemp.setName(item.getStudentName());
						majorRankTemp.setStudentNumber(user.getUserName());
						majorRankTemp.setGpa(item.getGpa());
						majorRankTemp.setGpaSort(item.getGpaSort());
						majorRankTemp.setComprehensivePerformance(item.getComprehensivePerformance());
						majorRankTemp.setComprehensivePerformanceSort(item.getComprehensivePerformanceRanking());
						allRankList.add(majorRankTemp);
					}
				}
			}
		}
	...
	public StudentScoreTemp getStudentScoreTempById(int studentId) {
		...
		if(null != winScoreList)
		{
			for(WinScore item : winScoreList)
			{
				...
				winScoreTemp.setSiId(item.getScoreItems().getSiId());
		...
		//将winscoreTempList按照siId排序
		int sortNumber = 0;
		int index = 0;
		int i = 0;
		List<WinScoreTemp> listResult = new ArrayList<WinScoreTemp>();
		if(0 != winscoreTempList.size())
		{
			while(0 != winscoreTempList.size())
			{
				i = 0;
				index = 0;
				sortNumber = winscoreTempList.get(0).getSiId();
				for(WinScoreTemp item :winscoreTempList)
				{
					if(sortNumber > item.getSiId())
					{
						sortNumber = item.getSiId();
						index = i;
					}
					i ++;
				}
				listResult.add(winscoreTempList.get(index));
				winscoreTempList.remove(index);
			}
		}
		//���Ͻ���
		...
		studentScoreTemp.setWinScoreTempList(listResult);					//���û�����Ϣ
	...
	public String studentRegister(List<StudentSimpleInf> studentSimpleInfs) {
		...
		for(StudentSimpleInf s : studentSimpleInfs){
			...
			if(userDao.getUserByUserName(s.getUserName())!=null) {
				result += s.getStudentName()+" "+s.getUserName()+" 学号已存在\n";
				continue;
			}
			...
			if(classes == null) {
				result += s.getStudentName()+" "+s.getUserName()+" 班级不存在\n";
				continue;
			}
			
			student.setClasses(classes);
			student.setUsers(users);
			student.setDormitoryNumber(s.getDormitoryNumber());
	...
	public List<Student> getAllClassStudent(int studentId) {
		...
		return studentDao.getStudentByClassAndSortByStudentNumber(classNumber);
	}
	...
	public List<Student> getMajorStudent(int studentId) {
		// TODO Auto-generated method stub
		Student student = studentDao.getStudentById(studentId);
		int gradeId = student.getClasses().getGrade().getGradeId();
		int majorId = student.getClasses().getMajor().getMajorId();
		
		return studentDao.getStudentByGradeAndMajorSortByGPA(gradeId, majorId);
	}
